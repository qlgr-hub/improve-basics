...
bool ProtocalThread(reference_wrapper<Socket> connectionSocketRef)
{
    ...
    while (message != "QUIT") {
        ...
        if (message == "QUESTION") {
            ...
            string answer;
            answer = sstream.str().substr(static_cast<size_t>(sstream.tellg()) + 1); // 去掉消息头
    ...
    if (bindResult != -1) {
        ...
        if (listenResult != -1) {
            while (true) {
                ...
                async(launch::async, ProtocalThread, ref(acceptedSocket));
            }
        }
    }
